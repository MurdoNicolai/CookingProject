...
def RecipeBrowser():
    ...
    return html.div(
        ...
        html.div(
            ...
            # -------- RIGHT PANEL (DETAIL) --------
            # +++ CHANGED: wrap detail + fake scrollbar in a flex row +++
            html.div(
                {"class": "w-1/2 flex min-h-0 bg-beige-100 rounded-lg shadow-md border border-black"},

                # Scrollable content — native bar hidden
                html.div(
                    {
                        "class": "flex-1 flex flex-col gap-6 overflow-y-scroll min-h-0 p-8 hide-scroll",
                        "id": "detail-scroll"
                    },
                    ...
                    html.div(
                        {"class": "w-96 h-72 bg-orange-200 rounded-lg flex items-center justify-center text-6xl"},
                        "🍔",
                    ),

                    # Ingredients
                    html.div(
                        {"class": "bg-white/60 rounded-lg border border-amber-900 p-4"},
                        html.h3({"class": "text-lg mb-2"}, "Ingredients"),
                        html.ul(
                            {"class": "text-sm space-y-1 font-serif italic"},
                            *[html.li(f"- {i}") for i in recipe_detail["ingredients"]],
                        ),
                    ),
                ...
                # Fake scrollbar track
                html.div(
                    {"class": "fake-track my-4 mr-2", "id": "detail-track"},
    ...
    return html.div(
        ...
        html.header(
            ...
            html.div(
                ...
                # Filters grid (same structure as RecipeBrowser)
                html.div(
                    {"class": "px-6 py-4 flex-1 min-w-0"},
                    html.div(
                        {"class": "grid grid-cols-5 gap-2 auto-rows-max"},
        ...
        # ================= MAIN =================
        # Each panel scrolls independently via overflow-y-auto + min-h-0
        html.div(
            {"class": "flex-1 flex px-12 gap-10 min-h-0 pb-4"},
            ...
            html.div(
                ...
                # Recipe Grid — scrollable independently
                html.div(
                    {"class": "flex-1 grid grid-cols-4 gap-6 overflow-y-auto min-h-0 auto-rows-max"},
                    *[RecipeCard(recipe) for recipe in recipes * 3],
                ),
            ),

            # -------- RIGHT PANEL (DETAIL) — scrollable independently --------
            html.div(
                {"class": "w-1/2 bg-beige-100 rounded-lg shadow-md border border-black p-8 flex flex-col gap-6 overflow-y-auto min-h-0"},
                ...
                html.div(
                    {"class": "bg-white/60 rounded-lg border border-amber-900 p-4"},
                    html.h3({"class": "text-lg font-serif italic mb-2"}, "Ingredients"),
                    html.ul(
                        {"class": "text-sm space-y-1 font-serif italic"},
                        *[html.li(f"- {i}") for i in recipe_detail["ingredients"]],
                    ),
